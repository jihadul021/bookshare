...
import { createAddress as saveAddress, createExchangeRequest, createOrder, getAddresses, getCheckoutSession, getMyBooks, verifyCoupon } from '../api';
...
const Checkout = ({ cart, onOrderSuccess, onBack, paymentReturn, clearPaymentReturn }) => {
  ...
  const loadAddresses = async () => {
    try {
      ...
      const pendingCheckoutRaw = sessionStorage.getItem(PENDING_CHECKOUT_STORAGE_KEY);
      if (pendingCheckoutRaw) {
        try {
          const pendingCheckout = JSON.parse(pendingCheckoutRaw);
          if (pendingCheckout.selectedAddressId || pendingCheckout.formData) {
            return;
          }
        } catch (storageError) {
          console.error('Failed to inspect pending checkout before loading addresses:', storageError);
        }
      }
  ...
  const buildPurchaseOrderData = useCallback(() => ({
    ...
    shippingAddress: formData,
    couponCode: couponCode || null
  }), [cart.items, couponCode, formData]);
  ...
  const finalizeStripeOrder = useCallback(async (stripeSessionId) => {
    setLoading(true);
    setError('');

    try {
      const checkoutSessionResponse = await getCheckoutSession(stripeSessionId);
      if (checkoutSessionResponse.data?.paymentStatus !== 'paid') {
        throw new Error('Card payment was not completed successfully');
      }
  ...
  useEffect(() => {
    const pendingCheckout = sessionStorage.getItem(PENDING_CHECKOUT_STORAGE_KEY);
    if (!pendingCheckout) {
      return;
    }

    try {
      const parsedCheckout = JSON.parse(pendingCheckout);

      if (parsedCheckout.formData) {
        setFormData((current) => ({ ...current, ...parsedCheckout.formData }));
      }

      if (typeof parsedCheckout.couponCode === 'string') {
        setCouponCode(parsedCheckout.couponCode);
      }

      if (parsedCheckout.couponData) {
        setCouponData(parsedCheckout.couponData);
      }

      if (typeof parsedCheckout.selectedAddressId === 'string') {
        setSelectedAddressId(parsedCheckout.selectedAddressId);
      }

      if (typeof parsedCheckout.useSavedAddress === 'boolean') {
        setUseSavedAddress(parsedCheckout.useSavedAddress);
      }

      if (typeof parsedCheckout.saveNewAddress === 'boolean') {
        setSaveNewAddress(parsedCheckout.saveNewAddress);
      }

      if (parsedCheckout.paymentMethod) {
        setPaymentMethod(parsedCheckout.paymentMethod);
      }
    } catch (storageError) {
      console.error('Failed to restore pending checkout:', storageError);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    if (paymentReturn.status === 'cancelled') {
      setError('Card payment was canceled. You are back on your checkout page.');
      clearPaymentReturn?.();
      return;
    }

    if (paymentReturn.status === 'success' && paymentReturn.sessionId) {
      finalizeStripeOrder(paymentReturn.sessionId);
    }
  }, [clearPaymentReturn, finalizeStripeOrder, paymentReturn]);
      ...
      <div className="checkout-container">
        <div className="checkout-content">
          ...
          <div className="checkout-form-section">
            <p className="error-message">No checkout items were found. Please return to your cart and try again.</p>
          </div>
        </div>
      </div>
    );
          ...
          <div className="checkout-form-section">
            <form onSubmit={handleSubmit}>
                    ...
                    <div className="payment-options">
                      ...
                      <label className="payment-option">
                        ...
                        <span className="payment-label">
                          ...
                          <small>You will continue on Stripe&apos;s secure payment page</small>
                        </span>
                      </label>
                    </div>
                  </div>
              ...
              {paymentMethod === 'card' && !exchangeMode && (
                <StripePayment
                  amount={totalAmount}
                  onBeforeRedirect={prepareCardCheckout}
